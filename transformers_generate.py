    ...
    with open(generation_writer, 'w') as file_writer:
        ...
        for step, inputs in enumerate(eval_dataset):
            ...
            for index, (label_token, label) in enumerate(args.verbalizer.items()):
                ...
                # label_token_list = re.findall(original_input, args.label_token)
                # assert 1 == len(label_token_list), f'There are more than one {args.label_token} in sentence : {original_input}'
